...
def klines(symbol):
    try:
        df = pd.DataFrame(client.get_historical_klines(symbol, '1h', '24h UTC'))
...
while True:
    list = ['DOGEUSDT', 'XRPUSDT', 'CHZUSDT', 'ATOMUSDT']
    for item in list:
        time.sleep(20)
        prt('Script is running')
        strategy(f'{str(item)}', qty=12)
